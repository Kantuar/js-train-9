...
function findElementGreaterThan(arr, num) {
  ...
  const firstElement = arr.find(element => element > num);
...
function findLastEvenNumber(arr) {
  ...
  return lastEvenNumber ||  'Парних чисел не знайдено';
...
function getPositiveArrayValues(arr) {
  ...
  const arrayValue = Array.from(arr.values());
  const positiveArrayValues = arr.filter(arrayValue => arrayValue > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const mapArray = flatArray.map(element => element ** 2);
  const filterArray = mapArray.filter(element => element > threshold);
  return filterArray;
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastElements.reduceRight((accumulator, currentValue) => {
    console.log(currentValue);
    return accumulator + currentValue;
  }, 0);
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap(item => Array(n).fill(item));
...
function findDuplicateElements(arr) {
  ...
  duplicateElements = arr.filter((element, index) => {
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((accumulator, product) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastIndex = arr.findLastIndex((element) => element < 0);
